...
if __name__ == '__main__':
    ...
    epoch = cfg.getint("hyperParam", "epoch")
    wbcaa_lambda = cfg.getfloat("hyperParam", "wbcaa_lambda")
    lr = cfg.getfloat("hyperParam", "lr")

    train_files = cfg.get("files", "train_files")
    test_files = cfg.get("files", "test_files")
    verify_files = cfg.get("files", "verify_files")

    do_verify = cfg.getboolean("others", "do_verify")
    log_appendix = cfg.get("others", "log_appendix")
    ...
    log.append("iteration times: " + str(epoch) + "\n")
    ...
    with tf.variable_scope("loss"):
        ...
        d = tf.to_double(tf.shape(output_layer)[1])
        WBCAA_loss = tf.cast(tf.reduce_sum(tf.pow(tf.subtract(cov(init_output_layer), cov(output_layer)), 2.)) / 4*d*d, tf.float32)  # 4 * depth^2

        loss = tf.reduce_mean(per_example_loss) + WBCAA_loss * wbcaa_lambda  # λ = 1e-2
        ...
        texts, labels = io_bert.read_100k(train_files)
        train_step = epoch * int(len(labels) / batch_size) - epoch
    ...
    with tf.Session() as sess:
        ...
        for j in range(epoch):
            ...
            for i in range(int(labels.size/batch_size) - 1):
                ...
                losses.append(l)
                accuracies.append(a)
                print("train准确率:{},损失函数:{},进度：{}/{}".format(a, l, num, epoch * int(labels.size / batch_size) - epoch))
        ...
        saver.save(sess, "saved_models2/weibo.ckpt")

        # test
        texts, labels = io_bert.read_100k(test_files)
        ...
        if do_verify:
            # verify
            texts, labels = io_bert.read_100k(verify_files)
            ...
            for t in texts:
                single_input_id, single_input_mask, single_segment_id = \
                    create_input.convert_single_example(max_seq_length,tokenizer, t)
                input_idsList.append(single_input_id)
                input_masksList.append(single_input_mask)
                segment_idsList.append(single_segment_id)
            ...
            print("verify最终准确率:{},损失函数:{},F1:{}:".format(accuracy, np.mean(losses), formula_1))
            log.append("verify accuracy: " + str(np.mean(accuracies)) + "\t" +
                       "verify loss: " + str(np.mean(losses)) + "\n")
            log.append("F1: " + str(formula_1) + "\t" + "recall: " + str(recall) + "\t"
